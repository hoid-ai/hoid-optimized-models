...
extern "C" __global__ void bias_residual_ln_w1280_k(
  ...
  int r=blockIdx.x, lane=threadIdx.x, base=r*1280;
  __shared__ __align__(16) __nv_bfloat16 x[1280];
  __shared__ float warp_s[4], warp_ss[4], mean_s, inv_s;
  float s=0.0f, ss=0.0f;
  for (int j=8*lane; j<1280; j+=1024) {
...
#pragma unroll
  ...
  __syncthreads();
  if(wid==0){ s=(l<4)?warp_s[l]:0.0f; ss=(l<4)?warp_ss[l]:0.0f; for(int d=16;d;d>>=1){s+=__shfl_down_sync(mask,s,d);ss+=__shfl_down_sync(mask,ss,d);} if(l==0){float m=s/1280.0f; mean_s=m; inv_s=rsqrtf(fmaxf(ss/1280.0f-m*m,0.0f)+1e-5f);} }
  __syncthreads();
