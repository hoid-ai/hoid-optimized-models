...
extern "C" __global__ void prepare_qkv_w4_direct_k(
    __nv_bfloat16* out_q,
    ...
    int slot = (int)blockIdx.x * 4 + warp; // four independent (token, head) warps per CTA
    int s = dyn_dims[18];
    if (slot >= s * 48) return;
    ...
    if (h < 32 + 8) {
        ...
        if (h < 32) {
            ...
            nw  = k_norm_w;
            dst = k_cache + ((long)kh * 9216 + pos) * 128;
        }
        ...
        __nv_bfloat16* dst = v_cache + ((long)vh * 9216 + pos) * 128;
