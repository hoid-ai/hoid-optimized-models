...
// Four consumer warps preserve the accepted per-query-head score and online-
// softmax order. Two producer warps fill the next ping-pong K/V tile with
// cp.async while consumers process the current tile. This neither repartitions
// the softmax reduction nor duplicates a query head's score pass.
extern "C" __global__ void dense_cached_attention_residue_pipeline_k(
  ...
  // Each of the 64 producer lanes moves one aligned 16-byte vector from K and V.
  ...
  for (int base = 0, stage = 0; base < residue_count; base += TILE_KEYS, stage ^= 1) {
    ...
    if (consumer) {
      // Build four independent QK chains from the already-staged keys. Keeping
      // the key accumulators live together gives the scheduler independent FMA
      // and shuffle work while preserving every key's lane/delta reduction order.
      ...
      // Consume scores strictly in the accepted key order. This leaves the
      // online-softmax and V accumulation recurrence bit-for-bit structured as
      // before; only the independent score work above has been rescheduled.
      #pragma unroll
